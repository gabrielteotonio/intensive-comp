...
ParallelColumn <- function(mat_1, mat_2, t = 1) {
  ...
  val <- foreach(i = 1:dim(mat_1)[1], .combine = "rbind") %:%
    foreach(j = 1:dim(mat_2)[2], .combine = "c") %dopar% {
      sum(mat_1[i,] * mat_2[,j])
    }
